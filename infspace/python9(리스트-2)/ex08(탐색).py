# 탐색하기
# 주어진 데이터 내에서 어떠한 특정한 값을 찾는 행위가 참색하는 것

# index() 메서드를 이용하여 일반적인 탐색이 가능하다.
list_word = ["gold","blue","red","yellow","green"]
search_word = input("찾고자 하는 단어를 입력해 주세요 : ")

if search_word in list_word : 
    index1 = list_word.index(search_word)
    print("찾고자 하는 단어가", index1,"번째 인덱스에 존재합니다.")
else :
...
# 루프를 빠젼나오고 루프를 다 했음에도 없다면 탐색키가 존재하지 않음을 의미한다.

def number_search(list, key):
    cnt = 0
    for i in range(len(list)):        
        if key == list[i]:  # key 값이 list 에 있다면
            cnt += 1   # 같은 값이 존재하면 cnt 값이 증가
    return cnt
...
for i in num_list :
    if i > 50:
        ...
        result.append(i)
print(result)
print("50 보다 큰 값은", cnt,"개 있습니다.")
print("-"*50)


# 숙제
# 바로 위에 있는 코드를 사용자로부터 키를 입력 받아서 키값 이상의 값들을 출력하고
# 그 키값 이상인 값들의 총 갯수를 구하는 프로그램을 작성해보시오.
result = []
cnt = 0
key_value = int(input("찾고자 하는 값을 입력하세요 : "))
for i in num_list :
    if i > key_value:
        cnt += 1
        result.append(i)
print(result)
print(key_value, "보다 큰 값은", cnt,"개 있습니다.")
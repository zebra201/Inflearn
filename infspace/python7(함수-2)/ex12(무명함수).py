...
# 무명함수는 함수명이 없고, 구현부만 존재한다. 파이썬에는 무명함수를 만들 때,
# lamda 키워드로 만들 수 있다.
...
def main():
    ...
    # 가장 많이 사용되는 곳은 GUI 프로그램에서
    # 이벤트를 처리하는 콜백 함수형태로 많이 사용된다.
    sum = lambda x, y : x + y
    print("람다식을 이용한 함수명, 두 정수의 합 : ", sum(10,50))
    print("람다식을 이용한 함수명, 두 정수의 합 : ", sum(10,150))
